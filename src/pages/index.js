...
export const api = {
  ...
  getUserReport: (all=false) => fetch(`${BASE_URL}/user-report${all?"?all=1":""}`, { headers: H() }).then(handle),
  ...
  getAlerts: (all=false) => fetch(`${BASE_URL}/alerts${all?"?all=1":""}`, { headers: H() }).then(handle),
